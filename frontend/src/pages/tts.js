...
export default function TTS() {
  ...
  return (<>
    ...
    <div className={s.screen}>
      <Drawer open={drawer} onClose={()=>setDrawer(false)} active="tts"/>
      ...
      <div>
        <header className={s.header}>
          ...
          <div className={s.title}>🎙️ TTS & Sonidos</div>
        </header>
          ...
          <section className={s.panel}>
            ...
            <div className={s.ttsArea}>
              ...
              <div className={s.voiceRow}>
                {VOICES.map((v,i)=>(
                  <button key={i} className={`${s.voiceBtn} ${voice===i?s.voiceBtnActive:''}`} onClick={()=>setVoice(i)}>{v}</button>
                ))}
              </div>
              <button className={s.btnSpeak} onClick={speak}>🔊 Reproducir ahora</button>
              {status && <div className={s.err} style={{background:'rgba(174,124,255,.1)',borderColor:'rgba(174,124,255,.2)',color:'#c9a9ff'}}>{status}</div>}
            </div>
          </section>
          <div className={s.row}>
            ...
            <div className={s.card}><h3>💬 Chat Reader</h3><p>Leer mensajes del chat en vivo según filtros.</p><span className={s.tag}>Config</span></div>
            <div className={s.card}><h3>⭐ VIP Voice</h3><p>Voz especial para gifts premium y MVPs.</p><span className={s.tag}>Premium</span></div>
            <div className={s.card}><h3>📢 Anuncios</h3><p>Mensajes automáticos cada N minutos del live.</p><span className={s.tag}>Config</span></div>
          </div>
        </main>
      </div>
    </div>
