...
export default function Batallas() {
  ...
  useEffect(() => {
    const t = setInterval(() => setProg(p => Math.min(100, p + Math.random()*2)), 3000)
  ...
  return (<>
    ...
    <div className={s.screen}>
      <Drawer open={drawer} onClose={()=>setDrawer(false)} active="batallas"/>
      ...
      <div>
        <header className={s.header}>
          ...
          <div className={s.title}>⚔️ Batallas</div>
        </header>
        <main className={s.secPage}>
          <section className={s.panel}>
            <div className={s.sec}><span className={s.dot}/> Bonus Mission</div>
            <p style={{fontSize:12,color:'#8f88a9',marginBottom:8}}>Progreso actual hacia la bonus mission activa.</p>
          ...
          <div className={s.row}>
            ...
            <div className={s.card}><h3>🔥 Boosters</h3><p>x2 activo. Expira en 4:32 min.</p><span className={s.tag}>x2</span></div>
            <div className={s.card}><h3>📊 Score Live</h3><p>846 pts propios vs 1,203 rival. Diferencia: -357.</p><span className={s.tag}>Battle</span></div>
